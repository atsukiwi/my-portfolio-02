...
const Layout: React.FC<LayoutProps> = ({ children, categories }) => {
    ...
    return (
        <div className="flex min-h-screen bg-background text-foreground">
            ...
            <aside
                ref={sidebarRef}
                className={`fixed left-0 top-0 z-40 h-full w-80 overflow-y-auto border-r border-gray-800 bg-background p-8 transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
                    } scrollbar-custom`}
            >
                <div className="mb-8">
                        ...
                        {categories.map((category) => (
                            <li key={category.id}>
                                <Link
                                    ...
                                    className={`block rounded-md px-3 py-2 text-base transition-colors hover:bg-gray-800 hover:text-foreground ${router.asPath === `/category/${category.id}`
            ...
            <div className="flex flex-1 justify-center">
                <main
                    ref={mainContentRef}
                    className="w-full max-w-6xl overflow-y-auto px-4 py-8 md:px-8 scrollbar-custom"
                >
                    {children}
                </main>
            </div>
